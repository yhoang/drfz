...
fccp <- ConsensusClusterPlus::ConsensusClusterPlus(t(codes),  maxK = k, reps = 100, pItem = 0.9, pFeature = 1, title = tempdir(), plot = NULL, verbose = FALSE, clusterAlg = "hc", innerLinkage = linkage, finalLinkage = "average", distance = "euclidean", seed = rand_seed_consensus)
...
codes_clust_out <- data.frame(cluster = codes_clust, cell_id = cell_id, sample_id = samp, stringsAsFactors = FALSE)
write.table(codes_clust_out, file = file.path(outdir, paste0(prefix, "codes_clustering.xls")), row.names=FALSE, quote=FALSE, sep="\t")
...
# get cluster frequencies
codes_freq_clust <- table(codes_clust)

# make data frame with labels
codes_labels <- data.frame(cluster = sort(unique(codes_clust)), label = sort(unique(codes_clust)), counts = as.numeric(codes_freq_clust))
codes_labels$proportions <- round(codes_labels$counts/sum(codes_labels$counts) * 100, 2)


write.table(codes_labels, file = file.path(outdir, paste0(prefix, "codes_clustering_labels.xls")), row.names=FALSE, quote=FALSE, sep="\t")
