...
### get full data from file index in database
##### @param
# table       project name
# fileidx     file index
# stain       vector for column names 
# cofactor    cofactor for asinh transformation asinh(x/cof)
...
### get only Variables from database ----------------------------------------------------
...
fcs$calc_triplot_quadrant <- function (
  ...
  if (!is.na(min.bin.count) & (length(which(tab > min.cells)) < min.bin.count)) {
    ...
    if (all(!is.na(prod.cutoff))) {
      ...
      # if mean is not even
      if (!is.even(mean.bin.x)) {mean.bin.x <- mean.bin.x + size.bin/2}
      if (!is.even(mean.bin.y)) {mean.bin.y <- mean.bin.y + size.bin/2}
