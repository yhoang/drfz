...
freq.table.name = sprintf("%sRdata/%s_%s_%s_quadrant_%s_cof%s",folder.path,project.name,subgroup,coverage,stat.info,cofactor)
...
for ( i in 1:nrow(sub.set)) {
  ...
  for ( v1 in 1:(len.col-1) ) {
    ...
    quad.oper <- foreach ( v2=(v1+1):len.col,.combine=cbind )  %dopar% {
      ...
      for ( v3 in 1:len.col ) {
        if ( all(v3 != c(v1,v2)) ) {
          ...
          quad.results = fcs$calc_frequencies(temp.data = sampl.data, 
                                                   output = freq.info, 
                                                   prod.cutoff = cutoffs[c(v1,v2,v3)])
          ...
          for ( v3 in 1:len.col ) {
            if ( all(v3 != c(v1,v2)) ) {
              label.vec = c(label.vec,
                            paste0(colvec[v1],".",colvec[v2],".",colvec[v3],".",freq.info,".","Q1"),
                            paste0(colvec[v1],".",colvec[v2],".",colvec[v3],".",freq.info,".","Q2"),
                            paste0(colvec[v1],".",colvec[v2],".",colvec[v3],".",freq.info,".","Q3"),
                            paste0(colvec[v1],".",colvec[v2],".",colvec[v3],".",freq.info,".","Q4")
              )
            }
          }
    ...
    printf("%s::%s::quadrants::%s::v1=%s[%s/%s] ready (it=%s)", i, sub.set$`Patient ID`[i], freq.info, colvec[v1],v1,len.col, it)
    print(proc.time() - ptm)
  } 
  quad.df[i,] = quad.file
...
saveRDS(quad.df, file=paste0(freq.table.name,".rds"))
printf("%s saved!",freq.table.name)
