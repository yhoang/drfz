...
############### go through training set and create absRange of triploTs
label.name = vector()
for ( i in 1:nrow(train.set)) {
  ...
  if (i==1) {
    ...
    col.vec = colnames(temp.data)[-match(c(feat.X,feat.Y),colnames(temp.data))]
    ### 
    for (name in col.vec) {
...
### write
write.csv(bind, file=sprintf("%s_TrainingSet_triploT_%s_%s_%s_cof%s_minc%s_absRange.csv",project.name,feat.X,feat.Y,calc.meth,cofactor,mincells),row.names = F)
