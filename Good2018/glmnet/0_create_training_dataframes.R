...
libraries = c("xlsx","RSQLite","dplyr","reshape2","ggplot2","limma","ggrepel")
...
if (TRUE) {
  ...
  for ( i in 1:nrow(train.set)) {
    ...
    matchmeta = match(pat.id, train.set$`Patient ID`)
    data.man$condition = train.set$`Relapse Status`[matchmeta]
...
train.set$`Relapse Status` = factor(train.set$`Relapse Status`)
color_conditions = c("#006837","#f46d43","#b2182b")
names(color_conditions) = levels(train.set$`Relapse Status`)
...
colnames(median.mat) = train.set$`Patient ID`
...
matchmeta = match(ggdf.mds$sample_id, train.set$`Patient ID`)
ggdf.mds$condition <- train.set$`Relapse Status`[matchmeta]
...
ggmds = ggplot(ggdf.mds, aes(x = MDS1, y = MDS2, color = condition)) +
  ...
  scale_color_manual(values = color_conditions) +
  coord_fixed()
ggsave(paste0(data.table.name,"_MDS.pdf"), width=15, height=7)
...
data.NRS = list()
for ( i in 1:nrow(train.set) ) {
  data.tmp = temp.data.all[which(temp.data.all[,1]==train.set$`Patient ID`[i]),(2:ncol(temp.data.all))]
  data.NRS[[i]] = NRS(data.tmp)
}
data.nrs = t(as.data.frame((data.NRS)))
rownames(data.nrs) = train.set$`Patient ID`
...
matchmeta = match(ggdf.nrs$sample_id, train.set$`Patient ID`)
ggdf.nrs$condition <- train.set$`Relapse Status`[matchmeta]
...
#cells_table=cells_table[-which(cells_table==0)]
ggdf.cellsbar = data.frame(sample_id = names(cells_table), cell_counts = as.numeric(cells_table) )
matchmeta = match(ggdf.cellsbar$sample_id, train.set$`Patient ID`)
ggdf.cellsbar$condition <- train.set$`Relapse Status`[matchmeta]
...
ggsave(paste0(data.table.name,"_barplot.pdf"),width=10,height=5)
