...
# go through subgroup set and load single files and merge into one temp.data.all -----------------
...
data.NRS = list()
for ( i in 1:nrow(sub.set) ) {
  data.tmp = temp.data.all[which(temp.data.all[,1]==sub.set$`Patient ID`[i]),(2:ncol(temp.data.all))]
