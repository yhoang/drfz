...
train.set$`Patient ID` = factor(train.set$`Patient ID`)
...
quad.table.name = sprintf("%s/Rdata/%s_Training_%s_quadrant_%s_cof%s",folder.path,project.name,quad.info,coverage,cofactor)
...
quad.df = data.frame( matrix(
  ,nrow = nrow(train.set)
  ,ncol = 109668  # m=39, with ncol = m * (m-1) * (m-2) * 0.5 * 4
  ,byrow = TRUE
...
for ( i in 1:nrow(train.set)) {
  quad.sample_id = c(quad.sample_id,as.character(train.set$`Patient ID`[i]))
  
  file.name = paste0(train.set$`Patient ID`[i],"_",project.name,".fcs")
  file.idx = fileID$file_ID[which(fileID$filename==file.name)]
  if (length(file.idx)==0) {
    file.name = paste0(train.set$`Patient ID`[i],"_",tolower(project.name),".fcs") # file names vary..
    file.idx = fileID$file_ID[which(fileID$filename==file.name)]
  }
  ...
  for ( v1 in 1:(len.col-1) ) {
    ...
    quad.oper <- foreach ( v2=(v1+1):len.col,.combine=cbind )  %dopar% {
      ...
      for ( v3 in 1:len.col ) {
        if ( all(v3 != c(v1,v2)) ) {
          sampl.data = temp.data.all[which(temp.data.all$file_id==train.set$`Patient ID`[i])
...
# create label vector in the last step ------------------------------------------
    ...
    printf("%s::%s::quadrants::%s::v1=%s[%s/%s] ready (it=%s)", i, train.set$`Patient ID`[i], quad.info, colvec[v1],v1,len.col, it)
    print(proc.time() - ptm)
  } 
  quad.df[i,] = quad.file
