...
freq.table.name = sprintf("%s/Rdata/%s_%s_%s_freq_%s_cof%s",folder.path,project.name,subgroup,coverage,stat.info,cofactor)
...
for ( i in 1:nrow(sub.set)) {
  ...
  for ( v1 in 1:(len.col-1) ) {
    ...
    printf("%s::%s::quadrants::%s::v1=%s[%s/%s] ready (it=%s)", i, sub.set$`Patient ID`[i], stat.info, colvec[v1],v1,len.col, it)
    print(proc.time() - ptm)
  } 
  quad.df[i,] = quad.file
